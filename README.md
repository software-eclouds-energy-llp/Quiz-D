...
- **Randomized answer order** — each time a room is created, every
  question's four options are shuffled into a fresh order (the correct
  answer moves with its text), so it's not predictably in the same slot
  game after game. The shuffle happens once, at room creation, and is
  embedded in the room — so the host and every player see the exact same
  order for a given game, it's just different from the last game.
