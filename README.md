...
- **Automatic pacing** — the host no longer has to click through every
  ...
  manual override.
- **See who answered what** — on the host's reveal screen, a "👀 See who
  answered what" toggle expands each option's vote bar into the actual
  list of players who picked it (and who didn't answer at all). Hidden
  during live voting and only ever shown after the reveal, so it doesn't
  spoil anything or pressure anyone mid-question.
- **Game History** — a new tab in `admin.html` lists past games and lets
  you reopen any of them: final scoreboard, dare result, and every
  question's full answer breakdown, long after the game itself has ended.
...
The `.indexOn` entry isn't strictly required — the **Game History** tab in
`admin.html` sorts by `createdAt` client-side either way — but without it
Firebase logs a "no index defined" warning and the sort gets slower once
you've got a lot of past games. Cheap to add now.
...
**Game History tab:**
- Lists the last 25 games played (newest first), reading straight from the
  same `rooms/{code}` data `host.html` writes — nothing is duplicated or
  logged separately.
- **View** opens a full record of that game: final scoreboard, the dare
  result if one was spun, and every question with a vote breakdown that
  shows exactly which players picked which option (and who never
  answered) — the same "who answered what" detail from the host's reveal
  screen, but for every question at once and without needing to tap
  anything to reveal it.
- **Delete** removes that game's record from Firebase permanently — handy
  for clearing out test runs or old games, since rooms otherwise persist
  indefinitely.
